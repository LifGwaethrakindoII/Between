...
public class InputMappingWindow : EditorWindow
{
	...
	private const string EDITOR_PREFS_KEY_MAPPING_PATH = "Path_InputMapping_File"; 																									/// <summary>Input Mapping's File path key for the EditorPrefs.</summary>
	...
	[MenuItem(INPUTMAPPINGWINDOW_PATH)]
	...
	private void Save()
	{
		InputMapping mapping = VoidlessJSONSerializer.DeserializeFromJSONFromTextAsset<InputMapping>(file);
		...
		{			
			...
			VoidlessEditorData.SaveString(EDITOR_PREFS_KEY_MAPPING_PATH, AssetDatabase.GetAssetPath(file));
	...
	{
		string path = VoidlessEditorData.LoadString(EDITOR_PREFS_KEY_MAPPING_PATH);

		InputMapping mapping = VoidlessJSONSerializer.DeserializeFromJSONFromPath<InputMapping>(path);
