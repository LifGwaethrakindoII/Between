...
{
[Serializable]
public class VoidlessEditorData
{
	private const string PATH_ROOT_FOLDER = "/Voidless Utilities/Editor Default Resources"; 	/// <summary>Default Root's Path.</summary>
	private const string NAME_DATA = "/Data_VoidlessEditor.txt"; 								/// <summary>JSON's file default name.</summary>

	private static VoidlessEditorData editorData; 												/// <summary>EditorData's internal static reference.</summary>
	...
	/// <summary>VoidlessEditorData's Constructor.</summary>
	private VoidlessEditorData()
	{
...
#region StringFloatMethods:
	...
	private static void Load()
	{
		...
		{
			editorData = VoidlessJSONSerializer.DeserializeFromJSONFromPath<VoidlessEditorData>(filePath);
			if(editorData == null) editorData = new VoidlessEditorData();
		}
		else
		{
			if(!directoryExists) Directory.CreateDirectory(folderPath);
			File.Create(filePath);
			editorData = new VoidlessEditorData();
	...
	{
		Load();
		//VoidlessEditorGUILayout.ShowStringStringDictionary(ref editorData.StringStringDictionary);
	}
}
}