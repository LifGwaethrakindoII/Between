...
public abstract class BaseWaypointGeneratorInspector<T, W> : Editor where T : BaseWaypointGenerator<W> where W : Waypoint
{
	...
	protected virtual void DrawListInquiry()
	{
		...
		{
			waypointGenerator.waypoints = VoidlessEditorGUILayout.ListField(LABEL_WAYPOINTS, waypointGenerator.waypoints, true);
			if(waypointGenerator.waypoints.Count == 0)
			{
				waypointGenerator.quantity = 0;
				waypointGenerator.UpdateWaypointGenerator();
			}
			VoidlessEditorGUILayout.Spaces(3);
		}
	}
	...
	protected virtual void DrawWaypointUpdateInquiry()
	{
		...
		VoidlessEditorGUILayout.Spaces(1);
	...
	protected virtual void DrawDebugWaypoints()
	{
		...
		VoidlessEditorGUILayout.Spaces(2);
	}
	...
	protected virtual void OnGizmosInspectorGUI()
	{
		...
		EditorGUILayout.Space();
		waypointGenerator.drawWhenSelected = EditorGUILayout.Toggle(BaseGizmosMonoBehaviour<T>.LABEL_DRAW_WHEN_SELECTED, waypointGenerator.drawWhenSelected);
		VoidlessEditorGUILayout.Spaces(2);
	...
	{
		waypointGenerator.overrideDrawWhenSelected = EditorGUILayout.Toggle(BaseGizmosMonoBehaviour<T>.LABEL_DRAW_WHEN_SELECTED, waypointGenerator.overrideDrawWhenSelected);
