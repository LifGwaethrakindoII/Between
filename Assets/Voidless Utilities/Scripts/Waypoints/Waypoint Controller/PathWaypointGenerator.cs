...
public class PathWaypointGenerator : BaseWaypointGenerator<PathWaypoint>
{
	...
	protected virtual void DrawDebugCurvePath(int _index)
	{
		...
		switch(waypoints[_index].curveType)
		{
			...
			VoidlessGizmos.DrawCuadraticBeizerCurve(waypoints[_index].position, waypoints[nextIndex].position, waypoints[nextIndex].startTangent, timeSplit, pathLineColor);
			break;

			case CurveType.Cubic:
			VoidlessGizmos.DrawCubicBeizerCurve(waypoints[_index].position, waypoints[nextIndex].position, waypoints[nextIndex].startTangent, waypoints[nextIndex].endTangent, timeSplit, pathLineColor);
			break;
		}
	}
#endif
