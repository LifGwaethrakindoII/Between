...
#region Getters/Setters:
	...
	public FuzzySystem(FloatRange _range)
	{
	...
	public FuzzySystem(params FuzzySubset<T>[] _fuzzySet)
	{
		Type enumType = typeof(T);
		if(!enumType.IsEnum) throw new ArgumentException(NOT_ENUM);
		setsLength = Enum.GetValues(typeof(T)).Length;
	...
	private void CreateFuzzySetAndMapping()
	{
		...
		for(i = 0; i < setsLength; i++)
		{
			if(i < (setsLength - 1))
			{
				intersectionPoint = VoidlessMath.CalculateIntersectionBetween(fuzzySet[i].GetMaxRangeRay(), fuzzySet[i + 1].GetMinRangeRay());
	...
	private float? Defuzzyficate(FuzzySubset<T> _fuzzySubset, float _input)
	{
		...
		{ /// In Core Region.
			return 1.0f;
		} else if(_input > _fuzzySubset.range.min && _input < Mathf.Min(_fuzzySubset.valueRange.min, _fuzzySubset.valueRange.max))
		{ /// In Boundary Region.
			return MembershipFunction(_fuzzySubset.range.min, Mathf.Min(_fuzzySubset.valueRange.min, _fuzzySubset.valueRange.max), _input);

		} else if(_input > Mathf.Max(_fuzzySubset.valueRange.min, _fuzzySubset.valueRange.max) && _input < _fuzzySubset.range.max)
		{ /// In Boundary Region.
			return MembershipFunction(_fuzzySubset.range.max, Mathf.Max(_fuzzySubset.valueRange.min, _fuzzySubset.valueRange.max), _input);
		} /// In Support Region.
		else return null;
	}
