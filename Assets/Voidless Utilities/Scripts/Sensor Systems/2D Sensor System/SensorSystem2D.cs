		...
		foreach(SensorSubsystem2D subsystem in subsystems)
		{
			...
			foreach(SensorData2D sensorData in subsystem)
			{
				...
				switch(sensorData.sensorType)
				{
					...
					case SensorType.Box:
					Gizmos.DrawRay(sensorRay2D.origin, sensorDirection);
					VoidlessGizmos.DrawWireBox(sensorRay2D.origin + sensorDirection, sensorData.dimensions, transform.rotation);
					break;
					...
					VoidlessGizmos.DrawCapsule(sensorRay2D.origin, sensorDirection, sensorData.radius, sensorData.distance);
					break;
				}
			}
		}
	...
	public bool GetSubsystemDetection(int _subsystemIndex, out RaycastHit2D _hit)
	{
		...
		foreach(SensorData2D sensorData in subsystem)
		{
			...
			switch(sensorData.sensorType)
			{
				...
				case SensorType.Box:
				_hit = Physics2D.BoxCast(ray.origin, sensorData.dimensions, transform.rotation.eulerAngles.z, ray.direction, sensorData.distance, subsystem.layerMask);
				break;

				case SensorType.Sphere:
				_hit = Physics2D.CircleCast(ray.origin, sensorData.radius, ray.direction, sensorData.distance, subsystem.layerMask);
				break;

				case SensorType.Capsule:
				_hit = Physics2D.CapsuleCast(ray.origin, sensorData.dimensions, CapsuleDirection2D.Horizontal, transform.rotation.eulerAngles.z, ray.direction, sensorData.distance, subsystem.layerMask);
				break;
			}
	...
	public bool GetSubsystemDetection(int _subsystemIndex, out RaycastHit2D[] _hits)
	{
		...
		foreach(SensorData2D sensorData in subsystem)
		{
			...
			switch(sensorData.sensorType)
			{
				...
				case SensorType.Box:
				_hits = Physics2D.BoxCastAll(ray.origin, sensorData.dimensions, transform.rotation.eulerAngles.z, ray.direction, sensorData.distance, subsystem.layerMask);
				break;

				case SensorType.Sphere:
				_hits = Physics2D.CircleCastAll(ray.origin, sensorData.radius, ray.direction, sensorData.distance, subsystem.layerMask);
				break;

				case SensorType.Capsule:
				_hits = Physics2D.CapsuleCastAll(ray.origin, sensorData.dimensions, CapsuleDirection2D.Horizontal, transform.rotation.eulerAngles.z, ray.direction, sensorData.distance, subsystem.layerMask);
				break;
			}
